...
async function htmlToDom(html) {
  ...
  title =
    document.getElementById("schedule").firstElementChild.firstElementChild
      .firstElementChild;
  main = document.getElementById("main");
  main.insertBefore(title, main.firstChild);
  //Place a duplicate of the current week plan in current week section
  currentWeekPlan = document.querySelector(
    'h2[id$="vecka' + window.weekNumber + '"]:not([id^="flow"]'
  ).parentElement;
...
async function assingmentsToDom(assignments) {
  assignments.forEach((assignment) => {
    ...
    assignmentListWeek = assignmentList.querySelector(
      '[id$="' + weekID + '-assignment-list"]'
    );
    if (assignmentListWeek == null) {
      //If week does not exist in assignment list create it
...
fetchMarkdown(url).then((response) => {
  mdToGroupedHtml(response).then((response) => {
    htmlToDom(response).then((response) => {
      mermaid.init();
      addEditLink(courseParam);
      body = document.createElement("body");
      body.innerHTML = schedule.innerHTML;
      getAssignments(body).then((assingments) => {
