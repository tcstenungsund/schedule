      ...
      for (const group of groups) {
        ...
        getCourses(group).then((courses) => {
          for (const course of courses) {
            ...
            fetchMarkdown(urlPrefix + course.id + ".md").then((md) => {
              //Convert markdown to html
              mdToGroupedHtml(md).then((html) => {
                //put html from md in body so that assingments can be found
                (body = document.createElement("body")), body.appendChild(html);
                ...
                getAssignments(body).then((assignments) => {
                  for (const assignment of assignments) {
                    ...
                    assignmentWeek = assignment
                      ...
                      .split(" ")[1];
                    time.appendChild(
                      document.createTextNode("v" + assignmentWeek)
                    );
